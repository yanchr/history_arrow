...
document.addEventListener('DOMContentLoaded', () => {
	...
	function updateCoords(evt) {
		...
		current_year = convert_x_to_years(svgP.x, inputYear.value * magnitudeValue, isAbsolute);
		...
		if (viewing_style_checkbox.checked) {
			if (current_year < 0) {
	...
	window.addEventListener('magnitudeChanged', (e) => {
        ...
        yearValue = (isAbsolute) ? actual_currentYear - e.detail.first_year : e.detail.first_year;
        magnitudeValue = e.detail.magnitude;
